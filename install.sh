...
unset LD_PRELOAD
if [ "\$1" = "install" ]; then
	nh -r apt remove mitmproxy -y && nh -r apt update && nh -r apt install tightvncserver kali-desktop-lxde lxde-core kali-menu net-tools lxterminal -y && nh -r dpkg --configure -a;
fi
if [ "\$1" = "stop" ]; then
