...
function fix_sudo() {
    ...
    if [ -f "$CHROOT/usr/bin/sudo" ]; then chmod +s $CHROOT/usr/bin/sudo; else nh -r /bin/apt update && nh -r /bin/apt install sudo busybox -y && nh -r /sbin/useradd -m $USERNAME && nh -r /sbin/usermod -g sudo $USERNAME && chmod +s $CHROOT/usr/bin/sudo; fi
