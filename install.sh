...
function fix_sudo() {
   ...
   if [ ! -f "$CHROOT/usr/bin/sudo" ]; then /data/data/com.termux/files/usr/bin/bash -c "$CHROOT/usr/bin/apt install sudo -y"; else chmod +s $CHROOT/usr/bin/sudo; fi
   if [ ! -f "$CHROOT/usr/bin/su" ]; then /data/data/com.termux/files/usr/bin/bash -c "$CHROOT/usr/bin/apt install busybox -y"; else chmod +s $CHROOT/usr/bin/su; fi
