...
function fix_sudo() {
   ...
   if [ ! -f "$CHROOT/usr/bin/sudo" ]; then proot -b $CHROOT/usr/bin/bash -c "apt install sudo -y"; else chmod +s $CHROOT/usr/bin/sudo; fi
   if [ ! -f "$CHROOT/usr/bin/su" ]; then proot -b $CHROOT/usr/bin/bash -c "apt install busybox -y"; else chmod +s $CHROOT/usr/bin/su; fi
