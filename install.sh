...
unset LD_PRELOAD
if [ "\$1" = "install" ]; then
	nh -r apt remove mitmproxy -y && nh -r apt update && nh -r apt install tightvncserver kali-desktop-lxde kali-menu net-tools lxterminal -y && nh -r apt dist-upgrade -y;
fi
if [ "\$1" = "stop" ]; then
