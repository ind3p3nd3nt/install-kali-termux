...
function fix_sudo() {
   ...
   if [ ! -f "$CHROOT/usr/bin/sudo" ]; then ./$CHROOT/usr/bin/apt install sudo -y; else chmod +s $CHROOT/usr/bin/sudo; fi
   if [ ! -f "$CHROOT/usr/bin/su" ]; then ./$CHROOT/usr/bin/apt install busybox sudo -y; else chmod +s $CHROOT/usr/bin/su; fi
